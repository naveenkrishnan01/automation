...
t = browser.div(:class => "homepage_image featured_story1")
 if t.exists?
    puts "Featured Image found for Books and clicked on the feature".green
 else
    puts "Feature Image not found for Books Page".red
 end
...
m = browser.div(:class => "promo_image")
  if m.exists?
   puts "Promo Image found for Books and clicked on the promo".green
  else
    puts "Promo Image not found for Books Page".red
  end
...
l = browser.element(:xpath => ".//*[@id='feed_results']/div[3]/div[2]/div[1]/a/img");
 if l.exists?
  puts "The article feed clicked on the Books page".green
 else
  puts "Article feed not found on Books page".red
 end

l.click; 
sleep 3;

puts "Test is now complete for Books Page".green
